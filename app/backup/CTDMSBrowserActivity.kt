...
import com.cumulations.libreV2.fragments.CTDMRBrowserFragment
...
import kotlinx.android.synthetic.main.ct_activity_upnp_browser.toolbar
...
class CTDMSBrowserActivity : CTDeviceDiscoveryActivity(), DMSProcessor.DMSProcessorListener, LibreDeviceInteractionListner {
    companion object {
        private val TAG = CTDMSBrowserActivity::class.java.name
    }

    private var currentFragment: CTDMRBrowserFragment? = null
    ...
    private val fragmentMap: HashMap<String, CTDMRBrowserFragment> = HashMap()
            ...
            when (msg.what) {
                MediaEnum.MEDIA_PROCESS_INIT -> {
                    ...
                    if (mNetIf == null) {
                        ...
                        startService(Intent(this@CTDMSBrowserActivity, LoadLocalContentService::class.java))
    ...
    private fun setupTabsViewPager() {
        ...
        view_pager_tabs.offscreenPageLimit = 3
    ...
    fun browse(didlObject: DIDLObject?) {
        ...
        if (id == ContentTree.ROOT_ID) {
            updateTitle(getString(R.string.browseSongs))
        } else {
    ...
    fun play(didlObject: DIDLObject) {
        ...
        closeKeyboard(this@CTDMSBrowserActivity, currentFocus)
    ...
    override fun onBrowseComplete(parentObjectId: String?, result: Map<String, List<DIDLObject>>) {
        ...
        runOnUiThread {
            ...
            if (containersList?.isNotEmpty()!!) {
                for (container in containersList) {
                    ...
                    when (container?.title) {
                        ALBUMS -> {
                            val fragment = fragmentMap[ALBUMS]
                            fragment?.provideContainer(container)
                            fragmentMap[ALBUMS] = fragment!!
                        }

                        ARTISTS -> {
                            val fragment = fragmentMap[ARTISTS]
                            fragment?.provideContainer(container)
                            fragmentMap[ARTISTS] = fragment!!
                        }

                        SONGS -> {
                            val fragment = fragmentMap[SONGS]
                            fragment?.provideContainer(container)
                            fragmentMap[SONGS] = fragment!!
                        }
                    }
                }
            }
            ...
            /*if (currentFragmentDIDLObjectList.isEmpty()) {
    ...
    override fun onStartComplete() {
        ...
        if (dmsDevice == null) {
            ...
            upnpProcessor!!.searchDMR()
            val builder = AlertDialog.Builder(this@CTDMSBrowserActivity)
...
//            parseJsonAndReflectInUI(message)
    ...
    fun handleDIDLObjectClick(clickedDIDLPosition: Int) {
        ...
        this@CTDMSBrowserActivity.position = clickedDIDLPosition

        Log.i(TAG, "handleDIDLObjectClick position:" + this@CTDMSBrowserActivity.position)
    ...
    private fun openNowPlaying(setDMR: Boolean) {
        ...
        val intent = Intent(this@CTDMSBrowserActivity, CTNowPlayingActivity::class.java)
    ...
    inner class TabsViewPagerAdapter(fm: FragmentManager) : FragmentPagerAdapter(fm) {
        ...
        override fun getItem(position: Int): CTDMRBrowserFragment {
            when (tabTitles[position]) {
                ALBUMS -> {
                    if (fragmentMap[ALBUMS] == null) {
                        fragmentMap[ALBUMS] = CTDMRBrowserFragment().apply {
                ...
                ARTISTS -> {
                    if (fragmentMap[ARTISTS] == null) {
                        fragmentMap[ARTISTS] = CTDMRBrowserFragment().apply {
                            arguments = Bundle().apply {
                ...
                SONGS -> {
                    if (fragmentMap[SONGS] == null) {
                        fragmentMap[SONGS] = CTDMRBrowserFragment().apply {
                            arguments = Bundle().apply {
                ...
                GENRES -> {
                    if (fragmentMap[GENRES] == null) {
                        fragmentMap[GENRES] = CTDMRBrowserFragment().apply {
                            arguments = Bundle().apply {
    ...
    fun getCurrentFragment(): CTDMRBrowserFragment? {
        val currentFragmentIndex = view_pager_tabs?.currentItem
        if (currentFragmentIndex!! >= 0) {
            val fragment = supportFragmentManager?.fragments?.get(currentFragmentIndex) as CTDMRBrowserFragment?
            if (fragment != null) {
