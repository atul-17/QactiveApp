...
/**
 * Created by Amit T on 12-08-2017.
 */

public class DummyVisualizer extends LinearLayout implements IVisualizer {
    ...
    private boolean modfiywaves;
    private Handler wavesUiHandler = new Handler();
    private Runnable wavesUiRunnable = new Runnable() {
        @Override
        public void run() {

            if (modfiywaves) {
                float pitchInHz = getRandomFreq(10, 70);
                ...
                for (int i = 0; i < waveList.size(); i++) {
                    ...
                    /*To slow down wave/bar views jump, almost no effect*/
                    /*ScaleAnimation scale = new ScaleAnimation((float)1.0, (float)1.5, (float)1.0, (float)1.5);
                    scale.setFillAfter(true);
                    scale.setDuration(2000);
                    waveList.get(i).startAnimation(scale);*/
                }
                wavesUiHandler.post(wavesUiRunnable);
            } else {
                // do nothing
            }
        }
    ...
    public DummyVisualizer(Context context) {
        super(context);
    ...
    public DummyVisualizer(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
    ...
    public DummyVisualizer(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void showWaves(boolean modify) {
        modfiywaves = modify;
        wavesUiHandler.post(wavesUiRunnable);
    }

    private float getRandomFreq(int lowest, int highest) {
        Random r = new Random();
