...
public class MFAActivity extends AppCompatActivity {
            ...
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
                if (s.length() == 0) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        mfaInput.setBackground(getResources().getDrawable(R.drawable.text_border_selector));
                    }
                }
            }
    ...
    private void getCode() {
        ...
        if (MFACode == null || MFACode.length() < 1) {
            ...
            mfaInput.setBackground(getResources().getDrawable(R.drawable.text_border_error));
            return;
        }
        exit(MFACode);
    }
